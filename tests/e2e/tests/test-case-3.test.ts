...
async function runLoomDownload({
  ...
  try {
    ...
    await previewWrapper.hover({ timeout: 30_000 }); // Overlay buttons only actionable while preview is hovered
...
test.describe('Loom case 3', () => {
  test.use({ caseId: 'loom-about-insightai' });

  test('downloads Loom about page video end-to-end', async ({
    page,
    context,
    extensionId,
    caseData,
