...
async function runLoomDownload({
  ...
  try {
    ...
    await previewWrapper.hover({ timeout: 30_000 }); // Preview overlay gates download action on hover

    // Disable concurrent fragment downloads for this case to align with profile expectations
    await panelPage.evaluate(() => {
      type PanelStateShim = {
        mediaItems?: Map<string, unknown> & {
          values: () => IterableIterator<unknown>;
        };
      };
      const state = (window as unknown as { __SERP_PANEL_STATE__?: PanelStateShim }).__SERP_PANEL_STATE__;
      if (!state || !state.mediaItems || typeof state.mediaItems.values !== 'function') {
        return;
      }
      const iterator = state.mediaItems.values();
      const next = iterator.next();
      if (!next || next.done || !next.value) {
        return;
      }
      const entry = next.value as { recipeParams?: Record<string, unknown> };
      entry.recipeParams = {
        ...(entry.recipeParams ?? {}),
...
test.describe('Loom case 2', () => {
  test.use({ caseId: 'loom-community-mini-ai-lead-accelerator' });

  test('downloads Loom community post video end-to-end', async ({
    page,
    context,
    extensionId,
    caseData,
